...
**MVP Scope (Phase 2)**: User Story 1 only - Rule-based categorization with merchant dictionary  
**Full Feature**: Complete all three user stories + pluggable LLM provider abstraction
...
**Purpose**: LLM provider abstraction, database schema, and configuration management - MUST COMPLETE before user story implementation
...
- [ ] T011 Create LLMProvider interface in `backend/internal/llm/provider.go` with `Generate(ctx, req) -> (resp, error)` method and supporting types (GenerateRequest, GenerateResponse)
- [ ] T012 Create provider factory in `backend/internal/llm/factory.go` with `NewProvider(name, opts)` and `FromEnv()` functions; implement functional options pattern
- [ ] T013 [P] Create Ollama provider implementation in `backend/internal/llm/ollama.go`: HTTP client, request/response mapping, retry logic with 3 attempts (1s, 2s, 5s backoff)
- [ ] T014 [P] Create Claude provider stub in `backend/internal/llm/claude.go`: HTTP client setup, request/response mapping (will be completed in Phase 4)
- [ ] T015 [P] Create mock provider in `backend/internal/llm/mock.go` for testing

### Configuration Management

- [ ] T016 Create config struct in `backend/internal/config/llm_config.go`: ProviderConfig, LLMConfig, validation logic
- [ ] T017 Create config loading in `backend/internal/config/loader.go`: Viper setup, environment variable binding, startup validation
- [ ] T018 Create `backend/config/llm-config.yaml`: Provider blocks for Ollama (enabled, mistral:7b), Claude (disabled), OpenAI (disabled)
- [ ] T019 Create startup validation in `backend/cmd/statement-import-api/main.go`: Load LLMConfig, validate enabled provider, test connectivity, fail fast on config errors
- [ ] T020 [P] Add environment variable documentation: Update `backend/.env.example` with LLM_PROVIDER, OLLAMA_BASE_URL, ANTHROPIC_API_KEY, OPENAI_API_KEY
...
- [ ] T023 Create confidence scoring in `backend/internal/categorization/confidence.go`: Score mapping for rule-based (1.0), fuzzy (0.85-0.99), LLM (provider-specific)
...
- [ ] T027 [US1] Seed merchant dictionary in `backend/db/seeds/merchant_dictionary_seed.sql`: Insert 500+ entries (Swiggy→Food, Amazon→Shopping, Uber→Transport, etc.) for Indian banks
- [ ] T028 [US1] Implement rule-based categorization in `backend/internal/categorization/service.go`: CategorizeRule method using merchant dictionary exact + fuzzy matching, returns (category, confidence, explanation)
- [ ] T029 [US1] Implement /api/v1/transactions/categorize endpoint in `backend/internal/api/categorize.go`: POST handler accepting transactions array, returns categorizations with method/confidence/explanation
- [ ] T030 [P] [US1] Add transaction_id uniqueness constraint in transaction_categories table (via T008 migration update)
...
## Phase 4: User Story 2 - LLM-Based Categorization for Unknown Merchants (Priority: P2)
...
## Phase 5: User Story 3 - Category Management & Analytics (Priority: P3)
...
| **2** | Foundational (LLM + DB + Config) | T006-T023 | Phase 1 |
| **3** | US1: Rule-Based (P1 MVP) | T024-T038 | Phase 2 |
| **4** | US2: LLM Categorization (P2) | T039-T055 | Phase 2, Phase 3 |
| **5** | US3: Analytics & Recategorize (P3) | T056-T073 | Phase 2, Phase 3 |
| **6** | Polish & Cross-Cutting | T074-T093 | Phase 3, 4, 5 |

**Total Tasks**: 93  
**MVP Tasks** (Phase 1 + 2 + 3 only): 38  
**Full Feature**: 93
...
2. **Phase 2**: Foundational (T006-T023, focus on DB schema and basic config)
3. **Phase 3**: User Story 1 only (T024-T038, rule-based categorization)

**What's NOT in MVP**:
- LLM categorization (Phase 4)
- Analytics & recategorization (Phase 5)
- Comprehensive testing (Phase 6)

**MVP Deliverable**: Users upload statements → transactions automatically categorized by merchant dictionary → categories shown in preview → categories persisted → basic contract tests passing
...
- Ollama running locally for LLM testing (can be mocked in tests)

**Blocking Dependencies**:
- Phase 2 (Foundational) MUST complete before Phase 3-5 (User stories)
- Database migrations (T006-T010) MUST run before any categorization logic

**Non-Blocking**:
- Frontend (T036-T073) can develop in parallel with backend once API contracts defined
- Tests (T032-T051, T063-T067) can run against API stubs once endpoints defined
